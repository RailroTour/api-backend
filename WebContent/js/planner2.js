...
$(document).ready(function(){
    ...
    $(document).on("click", ".areas>.area", function(){ //여행 지역 추가
        ...
        $(".selected-area>.vals").append('<div class="area-item" data-area='+area_code+' data-sigungu='+sigungu_code+'><button class="cancel-btn"><img src="./jpg/cancel_btn.png" alt=""></button><span class="area-name">'+area_name+'</span><button class="adjust-btn minus"><img src="./jpg/minus.png" alt=""></button><span><input type="text" disabled value="7" class="day-num"> 일</span><button class="adjust-btn plus"><img src="./jpg/plus.png" alt=""></button></div>');
    })
	...
	$(".detail-schedule-btn").on('click', function(){
		$.ajax({
	...
	$.ajax({
		...
		success: function(data) {
			...
			for(var i=0; i<data.length; i++){
				$(".areas").append('<li class="area" data-area='+data[i].area_code+' data-sigungu='+data[i].sigungu_code+'><img src='+data[i].thumb+' alt="" class="area-img"><div class="black-cover"></div><span class="do">'+data[i].area_name+',</span><span class="si-gun-gu">'+data[i].sigungu_name+'</span></li>');
			}
		},
		error: function(request, status, error) {
			//서버로부터 응답이 정상적으로 처리되지 못햇을 때 실행
