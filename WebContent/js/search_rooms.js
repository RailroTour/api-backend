...
$(document).ready(function(){
            ...
            success:function(data){
                ...
                for(var i=0; i<Math.ceil(total_cnt/10); i++){
                    if((i+1)==$.urlParam('pageNo')){
                        $(".page_num_group").append('<a href=search_rooms.jsp?keyword='+$.urlParam('keyword')+'&pageNo='+(i+1)+' class="selected">'+(i+1)+'</a>');
                    }
                    else{
                        $(".page_num_group").append('<a href=search_rooms.jsp?keyword='+$.urlParam('keyword')+'&pageNo='+(i+1)+'>'+(i+1)+'</a>');
                    }
                }
            },
            error:function(){
    ...
    $(".search_btn").on('click', function(){
        ...
        location.href="search_rooms.jsp?pageNo=1&keyword="+$(".search_input").val();
    });
});
