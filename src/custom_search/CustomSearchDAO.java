...
public class CustomSearchDAO {
	...
	public CustomSearchDAO(Connection conn) {
		this.conn = conn;
	}
	
	public List<MyPlannerBean> list(int days, int tema_id) {
		...
		try {
			String sql = "select user.id as user_id, \r\n"
					...
					+ "where planner.days = ? and planner.tema_id = ? group by planner.id order by planner.register_date desc;\r\n";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, days);
			pstmt.setInt(2, tema_id);
